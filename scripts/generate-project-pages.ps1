...
foreach ($Project in $Projects) {
  ...
  $MetaHtml = ($MetaRows | ForEach-Object { '<div><dt>{0}</dt><dd>{1}</dd></div>' -f (Escape-Html $_[0]),(Escape-Html $_[1]) }) -join ''

  $CreditRows = @(
    @('ROLE',$Project.Role), @('CATEGORY',$Project.Context), @('ORGANISATIONS',$Project.Organisations), @('OFFICE',$Project.Office)
  ) | Where-Object { $_[1] }
  $CreditHtml = ($CreditRows | ForEach-Object { '<div><dt>{0}</dt><dd>{1}</dd></div>' -f (Escape-Html $_[0]),(Escape-Html $_[1]) }) -join ''
  ...
  $Html = @"
<!doctype html>
<html lang="en" class="no-js" data-page="project" data-project-key="$(Escape-Html $Project.Slug)" data-initial-theme="$InitialTheme" data-site-theme="$InitialTheme" data-build="$Build">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
  <meta name="description" content="$(Escape-Html $Project.Description)">
  <meta name="robots" content="index, follow, max-image-preview:large">
  <meta name="theme-color" content="$ThemeColor">
  <meta property="og:type" content="article"><meta property="og:site_name" content="Ahmad Alhadidii">
  <meta property="og:title" content="$(Escape-Html $Project.SeoTitle)"><meta property="og:description" content="$(Escape-Html $Project.Description)">
  <meta property="og:url" content="$Canonical"><meta property="og:image" content="$ImageAbsolute"><meta property="og:image:alt" content="$(Escape-Html $Project.ImageAlt)"><meta property="og:image:width" content="$($Project.ImageWidth)"><meta property="og:image:height" content="$($Project.ImageHeight)">
  <meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="$(Escape-Html $Project.SeoTitle)"><meta name="twitter:description" content="$(Escape-Html $Project.Description)"><meta name="twitter:image" content="$ImageAbsolute"><meta name="twitter:image:alt" content="$(Escape-Html $Project.ImageAlt)">
  <title>$(Escape-Html $Project.SeoTitle)</title><link rel="canonical" href="$Canonical">
...
<body class="project-page" data-project="$(Escape-Html $Project.Slug)">
  ...
  <div class="site-shell"><header class="site-header" id="site-header"><div class="site-header__inner page-width"><a class="site-header__name" href="/#index" aria-label="Ahmad Alhadidii, return to portfolio index">AHMAD ALHADIDII</a><p class="running-header" aria-hidden="true"><span id="running-header-text">$(if($Project.Theme -eq 'manmatic'){'MANMATIC / HUMAN–MACHINE COLLABORATION / ACTIVE FIELD'}else{"PROJECT FILE $($Project.Number) / $($Project.Title) / $($Project.Year)"})</span></p><button class="nav-toggle" id="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-navigation" aria-label="Open navigation"><span class="nav-toggle__open">MENU</span><span class="nav-toggle__close">CLOSE</span></button><button class="nav-backdrop" type="button" data-nav-dismiss tabindex="-1" aria-label="Close navigation"></button><nav class="site-nav" id="primary-navigation" aria-label="Primary navigation"><a href="/#index"><span>INDEX</span></a><a href="/#profile"><span>PROFILE</span></a><a href="/#cv"><span>CV</span></a><a class="is-active" href="/#work" aria-current="location"><span>WORK</span></a><a href="/#computation"><span>COMPUTATION</span></a><a href="/#visual-studies"><span>VISUALS</span></a><a href="/#contact"><span>CONTACT</span></a><a class="site-nav__external" href="https://www.manmatic.institute/" target="_blank" rel="noopener noreferrer">MANMATIC FIELD <span aria-hidden="true">↗</span></a></nav></div></header>
    <main id="main-content" tabindex="-1"><article class="project-detail" id="project-detail"><div class="project-intro page-width"><header class="project-header" data-project-theme="$InitialTheme">$(if($Project.Slug -eq 'protocol-port'){'<div class="project-header__system-nav"><p>MANMATIC SYSTEM / PROTOCOL PORT</p><a href="/projects/manmatic/">BACK TO MANMATIC SYSTEM ←</a></div>'})<p class="project-header__eyebrow">PROJECT FILE $(Escape-Html $Project.Number) / $(Escape-Html $Project.Context)</p><h1 aria-label="$(Escape-Html $Project.Title)"><span class="project-header__archive-title pointer-scan">$(Escape-Html $Project.Title)</span><span class="project-header__archive-subtitle">$(Escape-Html $Project.Subtitle)</span></h1><p class="project-header__definition">$(Escape-Html $Project.Definition)</p><dl class="project-meta">$MetaHtml</dl></header><section class="project-hero" aria-label="Project visual"><figure class="project-hero__media image-frame image-reveal media--contain" style="--media-ratio:$($Project.ImageWidth) / $($Project.ImageHeight)"><div class="image-frame__crop"><img src="$($Project.Image)" width="$($Project.ImageWidth)" height="$($Project.ImageHeight)" alt="$(Escape-Html $Project.ImageAlt)" fetchpriority="high" decoding="async"></div><figcaption>$(Escape-Html $Project.Caption)</figcaption></figure></section></div>
      <section class="project-copy-section page-width"><p class="project-copy-section__label">01 / OVERVIEW</p><div class="project-copy-section__body"><h2>Project overview</h2><p>$(Escape-Html $Project.Overview)</p></div></section>$($Project.Extra)<section class="project-copy-section page-width"><p class="project-copy-section__label">03 / CREDITS</p><div class="project-copy-section__body"><h2>Project contribution</h2><dl class="project-credit-list">$CreditHtml</dl></div></section><nav class="project-navigation page-width" aria-label="Project navigation"><div class="project-navigation__top"><a href="/#work">BACK TO WORK ←</a><a href="/#index">PORTFOLIO INDEX ↑</a></div>$ProjectNav</nav></article></main>
    <footer class="site-footer page-width"><p>© 2026 AHMAD ALHADIDII</p><p>AS-SALT, JORDAN</p><a href="/#index">PORTFOLIO INDEX <span aria-hidden="true">↗</span></a></footer></div>
</body>
</html>
"@
